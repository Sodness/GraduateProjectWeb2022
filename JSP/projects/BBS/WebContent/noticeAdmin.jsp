		...
		<div class="top_box">
			...
			<div class="login_box">
				<form method="post" action="loginAction.jsp">
					...
					<input type="hidden" name="href" value="noticeAdmin"> 
			...
			%>
			<div class="login_box">
				<input type="button" value="로그아웃" onClick="location.replace('logoutAction.jsp?href=noticeAdmin')">
		...
		<div class="main_box">
			<div class="menu">
				<div class="submenu" style="background-color: #93c9eb;">
					<a class="underline" href='noticeAdmin.jsp' style="color: white;">공지사항</a>
				</div>
				<div class="submenu">
			...
			<div class=index>
				<ul>
					...
					%>
					<div class="list">
						<li class="list_wirter" style="border-right: 2px solid #ccc; border-bottom: 2px solid #ccc;"><%=noticeList.get(i).getNoticeID()%></li>
						<li class="list_title" style="border-right: 2px solid #ccc; border-bottom: 2px solid #ccc;">
						<a class="underline" href='post.jsp?category=noticeAdmin&bbsID=<%=noticeList.get(i).getNoticeID()%>'><%=noticeList.get(i).getNoticeTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>")%></a></li>
						<li class="list_wirter" style="border-right: 2px solid #ccc; border-bottom: 2px solid #ccc;"><%=noticeList.get(i).getUserID()%></li>
						<li class="list_date" style="border-bottom: 2px solid #ccc;"><%=noticeList.get(i).getNoticeDate().substring(0, 11)%></li>
					</div>
					<%
						} else {
					%>
					<div class="list">
						<li class="list_wirter" style="border-right: 2px solid #ccc;"><%=noticeList.get(i).getNoticeID()%></li>
						<li class="list_title" style="border-right: 2px solid #ccc;">
						<a class="underline" href='post.jsp?category=noticeAdmin&bbsID=<%=noticeList.get(i).getNoticeID()%>'><%=noticeList.get(i).getNoticeTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>")%></a></li>
						<li class="list_wirter" style="border-right: 2px solid #ccc;"><%=noticeList.get(i).getUserID()%></li>
						<li class="list_date"><%=noticeList.get(i).getNoticeDate().substring(0, 11)%></li>
					</div>
					<%
						}
					}
			...
			<div class="bottom_box">
				<div class=page>
					...
					%>
					<a class="page_num" href='noticeAdmin.jsp?pageNumber=<%=preNum%>'>이전</a>
					<%
					} else {
					%>
					...
					<ul class="pageNum">
						...
						if (1 != startPage) {
						%>
						<li><a href="noticeAdmin.jsp?pageNumber=<%=startPage - 1%>"></a></li>
						<%
						}
						int i = 0;
						...
						%>
						<li class="pageNo"><a href="noticeAdmin.jsp?pageNumber=<%=i%>"><%=i%></a></li>
						<%
							}
						}
						if (endPage != maxPage) { // endPage와 maxPage가 다르면 endPage+1이 가장 마지막
						%>
						<li class="pageNo"><a href="noticeAdmin.jsp?pageNumber=<%=endPage + 1%>"></a></li>
						<%
						}
						%>
					</ul>
					...
					if (maxPage > 10) { // maxPage가 10보다 클 경우만 다음 버튼 활성화, 클릭시 다음의 첫 페이지로 이동
						...
						if ((postNum) <= maxPage) {
					%>
					<a class="page_num" href='noticeAdmin.jsp?pageNumber=<%=postNum%>'>다음</a>
					<%
						} else {
					%>
				...
				<div class="button_box">
					<button onClick="location.href='write.jsp?category=noticeAdmin'" value="생성">생성</button>
				</div>
				<%
				}
				%>
